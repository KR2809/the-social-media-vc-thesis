/* View 1 — Pick. Portfolio + collapsible context sidebar with KG + audit. */

const { useState: useState1, useEffect: useEffect1, useMemo: useMemo1, useRef: useRef1 } = React;
const { Avatar, OutcomeChip, ScoreSpark, EpistemeBar, ViewIntro, InfoTip, fmtMoney, fmtScore } = window.Chrome;

// ---------- one founder row ----------
function FounderRow({ row, rank, alloc, focused, onClick, prevRank }) {
  const delta = prevRank == null ? 0 : prevRank - rank;
  return (
    <div className={"founder-row " + (focused ? "focused" : "")} onClick={onClick}>
      ...
      <div className="col-id">
        ...
        <div className="id-block">
          ...
          <span className="name">{row.name} · <span className="muted">{row.niche}</span></span>
        </div>
      </div>
      <div className="col-scores">
        ...
        <span className="spark-wrap"><ScoreSpark founderId={row.id} tNow={row._t}/></span>
      </div>
      <div className="col-alloc mono">{fmtMoney(alloc)}</div>
      <div className="col-outcome"><OutcomeChip outcome={row.outcome}/></div>
    </div>
  );
}
  ...
  return (
    <div className="audit">
      <div className="audit-list">
        {entries.length === 0 && <div className="audit-empty muted">— slider steady. no membership changes.</div>}
        {entries.slice(0, 8).map((e, i) => (
          <div key={i} className={"audit-row " + e.kind}>
            ...
            <span className="audit-meta mono muted">
              {e.kind === "enter" ? `entered #${e.rank + 1}` :
               e.kind === "exit"  ? `exited (was #${e.prev + 1})` :
                                    `Δ ${e.dRank > 0 ? "+" : ""}${e.dRank} → #${e.rank + 1}`}
            </span>
          </div>
        ))}
      </div>
...
// ---------- KG mini-map (cohort at date T) ----------
function KGMini({ rows, focusedId, onFocus }) {
  const size = 200;
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} className="kg-mini">
      ...
      {[0.4, 0.65, 0.9].map((r, i) => (
        <circle key={i} cx={cx} cy={cy} r={r * (cx - 6)} fill="none" stroke="var(--hairline-2)" strokeDasharray="2 3"/>
      ))}
      {rows.slice(0, 10).map((row, i) => {
        ...
        return <line key={"e"+i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="var(--accent)" strokeOpacity="0.16" strokeWidth="0.6"/>;
      })}
      {rows.map((row, i) => {
        ...
        return (
          <g key={row.id} onClick={() => onFocus(row.id)} style={{ cursor: "pointer" }}>
            <circle cx={x} cy={y} r={ptSize + 2} fill="var(--accent)" opacity={focused ? 0.32 : 0.10}/>
            <circle cx={x} cy={y} r={ptSize} fill={focused ? "var(--accent)" : "var(--ink-1)"} opacity={0.4 + row.combined * 0.6}/>
            {focused && <circle cx={x} cy={y} r={ptSize + 5} fill="none" stroke="var(--accent)" strokeWidth="1.2"/>}
          </g>
        );
      })}
      <text x={cx} y={cy + 3} textAnchor="middle" fontFamily="var(--mono)" fontSize="10" fill="var(--ink-3)" fontWeight="500">N={rows.length}</text>
    </svg>
  );
}

// ---------- ranked portfolio ----------
function Portfolio({ rows, capital, focusedId, onFocus, t, prevRanks }) {
  const allocs = useMemo1(() => {
    const total = capital * 1_000_000;
    return rows.map(r => total / rows.length); // equal-weight for demo
  }, [rows, capital]);
  return (
    <div className="portfolio">
      <div className="portfolio-head">
        ...
        <div className="col-scores">
          ...
          <InfoTip width={320}>
            <strong>T1 · Tier-1 score.</strong> Raw social-signal model (gradient-boosted classifier on dimensions <span className="mono">S1–S6</span> from the taxonomy). Range <span className="mono">[0,1]</span>.<br/><br/>
            <strong>T2 · Tier-2 score.</strong> KG-augmented re-ranker. Takes T1 + features extracted from the founder's ego-network (mentors, peers, distribution loops).<br/><br/>
...
// ---------- View 1 root ----------
function View1Replay({ t, K, capital, focusedId, setFocused, gotoView, revealed, setRevealed }) {
  const rows = useMemo1(() => THESIS.rankAt(t, K), [t, K]);
  const prevRows = useRef1(rows);
  const [audit, setAudit] = useState1([]);
  const prevRanks = useMemo1(() => {
    const m = {};
  ...
  useEffect1(() => {
    ...
    rows.forEach((r, i) => {
      ...
      if (pi >= 0 && Math.abs(pi - i) >= 3) entries.push({ kind: "shift", id: r.id, rank: i, dRank: pi - i });
    });
    if (entries.length > 0) {
  ...
  return (
    <section className="view view-1">
      <ViewIntro kicker="STEP 01 · PICK" title="Who would the framework have backed?">
        These are the top <span className="mono">{K}</span> founders ranked by combined score at <strong>{THESIS.fmtMonth(t)}</strong>.
        <strong> Drag the slider</strong> to replay the past — picks reshuffle in real time, with no information from after that date.
        <strong> Click any row</strong> to focus a founder; come back here later to drill in.
      </ViewIntro>

      <div className="grid-2">
        <div className="center">
          <Portfolio rows={rows} capital={capital} focusedId={focusedId} onFocus={setFocused} t={t} prevRanks={prevRanks}/>
          <div className="portfolio-foot">
            <span className="muted">{rows.length} picks · cohort pool N={THESIS.FOUNDERS_RAW.filter(f => THESIS.months(f.first) <= t).length}</span>
            <button
              className={"reveal-btn " + (canReveal ? "" : "disabled")}
              disabled={!canReveal}
              onClick={() => { setRevealed(true); gotoView(2); }}
...
window.View1Replay = View1Replay;
